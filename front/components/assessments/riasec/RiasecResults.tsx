...
export function RiasecResults({ scores, onRetake, onBack }: Props) {
  ...
  const top = ranked[0]
  /* Code de Holland — 3 premières lettres */
  const hollandCode = ranked.slice(0, 3).map(p => p.letter).join("")
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-3xl px-6 py-16">
        <div className="text-center">
          ...
          {/* Code de Holland */}
          <div className="mt-6 inline-flex items-center gap-3 rounded-2xl bg-white border border-slate-200 px-5 py-3">
            ...
            <div className="flex items-center gap-1.5">
              {ranked.slice(0, 3).map(p => (
                <span key={p.letter} className="flex h-8 w-8 items-center justify-center rounded-lg font-bold text-white text-[13px]"
                  style={{ background: p.tone }}>
                  {p.letter}
                </span>
              ))}
              <span className="ml-1 font-display text-[18px] font-bold text-slate-700">{hollandCode}</span>
            </div>
          </div>
        </div>
