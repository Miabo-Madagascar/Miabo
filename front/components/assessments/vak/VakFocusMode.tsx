...
import type { VakQuestion } from "../data/vakQuestions"
...
import { VakFocusTopBar } from "./VakFocusTopBar"
import { DiscFocusCTABar } from "../disc/DiscFocusCTABar"

interface Props {
  questions:   VakQuestion[]
...
export function VakFocusMode({ questions, answers, setAnswers, onFinish, onExit, likertStyle }: Props) {
  ...
  const q       = questions[i]
  const profile = VAK_PROFILES[q.type]
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <VakFocusTopBar i={i} questions={questions} answers={answers}
        onNavigate={setI} onExit={onExit} />

      {/* Zone de question */}
      <div className="relative z-10 mx-auto max-w-3xl px-6 pt-10 pb-40">
        <div className="flex items-center gap-4 mb-7">
          <VakProfileIllustration type={profile.letter} size={64} />
          <div>
