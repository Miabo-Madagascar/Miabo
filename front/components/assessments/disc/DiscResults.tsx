...
import { DiscInterpretationCard }  from "./DiscInterpretationCard"
...
export function DiscResults({ scores, onRetake, onBack }: Props) {
  ...
  /* Profils secondaires : inclus si leur score est ≥ 40 % du score dominant */
  const threshold  = top.score * 0.4
  const dominants  = ranked.filter(p => p.score >= threshold)
  const profileKey = dominants.map(p => p.letter).join("")
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-3xl px-6 py-16">
        ...
        <div className="text-center">
          ...
          <div className="mt-5 inline-flex items-center gap-3 rounded-2xl bg-white border border-slate-200 px-5 py-3">
            <span className="text-[11px] font-bold uppercase tracking-[.14em] text-slate-400">
              {dominants.length > 1 ? "Profil combiné" : "Profil"}
            </span>
            <div className="flex items-center gap-1.5">
              {dominants.map((p, i) => (
                <div key={p.letter} className="flex items-center gap-1.5">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg font-bold text-white text-[13px]"
                    style={{ background: p.tone }}>{p.letter}</span>
                  {i < dominants.length - 1 && <span className="text-slate-300 text-[12px]">+</span>}
                </div>
              ))}
              <span className="ml-1 font-display text-[18px] font-bold text-slate-700">{profileKey}</span>
            </div>
          </div>

          {dominants.length > 1 && (
            <p className="mt-3 text-[12px] text-slate-500 max-w-xs mx-auto">
              La lettre <strong>{top.letter}</strong> indique votre tendance principale.{" "}
              <strong>{dominants.slice(1).map(p => p.letter).join(" et ")}</strong>{" "}
              {dominants.length === 2 ? "est" : "sont"} votre influence secondaire.
            </p>
          )}
        </div>
        ...
        <div className="mt-12 rounded-3xl border border-slate-200 bg-white p-6">
          <h3 className="text-[11px] font-bold uppercase tracking-[.14em] text-slate-500 mb-5">Répartition complète</h3>
            ...
            {ranked.map(p => (
              <div key={p.letter}>
                ...
                <div className="h-2 w-full overflow-hidden rounded-full bg-slate-100">
                  <div className="h-full transition-all duration-700" style={{ width: `${p.pct}%`, background: p.tone }} />
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Interprétation comportementale — onglets pour les 4 profils */}
        <DiscInterpretationCard dominants={dominants} all={ranked} />
