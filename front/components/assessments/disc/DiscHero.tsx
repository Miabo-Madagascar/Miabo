import { DISC_PROFILES, DISC_ORDER } from "../data/discProfiles"
import { DISC_DESCRIPTION } from "../data/discQuestions"
...
export function DiscHero({ totalCount, durationMin = 5 }: Props) {
  return (
    <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white">
      ...
      <div className="relative p-8 sm:p-10">
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {DISC_ORDER.map(letter => {
            const p = DISC_PROFILES[letter]
            return (
              <div key={letter}
                className="flex items-center gap-3 rounded-xl bg-white/70 border border-slate-200/80 p-2.5">
                <DiscProfileIllustration type={letter} size={48} />
