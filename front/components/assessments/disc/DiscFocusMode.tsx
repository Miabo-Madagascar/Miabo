...
import { DiscLikert, LikertStyle } from "./DiscLikert"
...
export function DiscFocusMode({ questions, answers, setAnswers, onFinish, onExit, likertStyle }: Props) {
  ...
  const q       = questions[i]
  const profile = DISC_PROFILES[q.type]
  ...
  /* Navigation clavier : 1-5 pour répondre, ←→ pour naviguer */
  useEffect(() => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-b from-slate-50 to-white">
      {/* Halo ambiant coloré selon le profil actif */}
      ...
      <DiscFocusTopBar i={i} questions={questions} answers={answers}
        onNavigate={setI} onExit={onExit} />

      {/* Zone de question */}
      <div className="relative z-10 mx-auto max-w-3xl px-6 pt-10 pb-40">
        <div className="flex items-center gap-4 mb-7">
          <DiscProfileIllustration type={profile.letter} size={64} />
          <div>
